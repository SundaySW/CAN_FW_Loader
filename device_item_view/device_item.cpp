...
void DeviceItem::ProcessFinished(qint64 msec) {
    ...
    ui->elapsed_label->setText(tr("%1 sec").arg((float)msec/1000));
    ui->upload_btn->setEnabled(false);
    ui->cancel_btn->setEnabled(false);
...
void DeviceItem::SetReqData(const QString& fileName, std::optional<uchar> addr, uchar sw_version) {
    ui->file_name_label->setText(fileName);
    if(addr.has_value())
        ui->req_addr_label->setText(QString("New addr: 0x%1").arg(addr.value(),1,16));
